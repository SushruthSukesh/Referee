...
with col1:
    dataset_size = st.selectbox(
        "Dataset Size",
        ["Small (<10k rows)", "Medium (10k–1M rows)", "Large (>1M rows)"]
    )
...
st.subheader("📊 Model Comparison (Generated by Kiro)")
...
st.subheader("⚖️ Key Trade-offs (Kiro Analysis)")

for key, value in tradeoffs.items():
    st.info(f"**{key.replace('_', ' ').title()}**: {value}")


st.subheader("🧠 Referee Guidance (Conditional)")
...
try:
    st.subheader("📌 Additional Guidance (From Kiro)")
...
st.divider()
st.caption("Powered by Kiro • Built as a decision-support referee, not a recommendation engine")
